...
string path = @"c:\rapid_cord\flutter-webrtc\windows\flutter_webrtc_plugin.cc";
string content = File.ReadAllText(path);

string pattern = @"static DWORD WINAPI DuckPreventionThreadProc\(LPVOID\) \{.*?return 0;\s*\}";

string newProc = @"static DWORD WINAPI DuckPreventionThreadProc(LPVOID) {
              ...
              if (SUCCEEDED(client->GetMixFormat(&format))) {
                  if (SUCCEEDED(client->Initialize(AUDCLNT_SHAREMODE_SHARED, 0, 10000000, 0, format, nullptr))) {
                      ...
                      if (SUCCEEDED(client->GetService(__uuidof(IAudioSessionControl2), (void**)&control))) {
                          ...
                          // MUST START the dummy stream so Windows activates the session visibly and applies duck logic
                          client->Start();
...
content = Regex.Replace(content, pattern, newProc, RegexOptions.Singleline);
